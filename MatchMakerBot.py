...
def view(message):
    ...
    if user_info:
        ...
        response += f"Телеграмм : {user_info[10]}\n"
...
def collect_data(message):
    ...
    if 'update' in state:  # Логика для изменения данных
        if 'name' in state:
            ...
            conn.commit()
        elif 'telegramm' not in state:
            cursor.execute('UPDATE users SET telegramm = ? WHERE user_id = ?', (message.text, user_id))
            conn.commit()
        ...
        if 'name' not in state:
            ...
            bot.reply_to(message, "Введите ваш тэг в телеграмме:")
        elif 'telegramm' not in state:
            state['telegramm'] = message.text
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
                           (user_id, state['name'], state['surname'], state['year_of_birth'], state['city'],
                            state['phone_number'],
                            state['level'], state['leading_leg'], state['position'], state['skill'], state['telegramm']))
